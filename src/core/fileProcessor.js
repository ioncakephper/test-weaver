...
function processFile(yamlFilePath, cliConfig) {
  try {
    ...
    let outputDir;
    if (cliConfig.outputDir && cliConfig.outputDir !== '.') {
      const relativePath = path.relative(
        process.cwd(),
        path.dirname(yamlFilePath),
      );
      outputDir = path.join(cliConfig.outputDir, relativePath);
    } else {
      outputDir = path.dirname(yamlFilePath);
    }
    ...
    // Security check to prevent writing outside of the project directory
    const resolvedOutputFilePath = path.resolve(outputFilePath);
    const resolvedCwd = path.resolve(process.cwd());

    if (!resolvedOutputFilePath.startsWith(resolvedCwd)) {
      log(
        `❌ Error: Attempted to write to a location outside the project directory: ${resolvedOutputFilePath}`,
        LOG_LEVELS.ERROR,
      );
      return;
    }
